...
class SsoModel extends CModel{
    ...
    public function apiGetLogin(){
        $userInfo = empty($_COOKIE[$this->token]) ? '' : json_decode($this->decrypt($_COOKIE[$this->token]),true);
    ...
    public function apiGetWspLogin(){
        $userInfo = empty($_COOKIE[$this->wspToken]) ? '' : json_decode($this->decrypt($_COOKIE[$this->wspToken]),true);
    ...
    public function apiSetWspLogin(){
        ...
        $userInfo = $this->param;
        $userInfo = $this->encrypt(json_encode($userInfo));
        setcookie($this->wspToken,$userInfo,time()+604800,'/',CDOMAIN);
        return array('Flag'=>100,'FlagString'=>'登陆成功');
    }
}