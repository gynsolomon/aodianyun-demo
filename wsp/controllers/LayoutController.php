...
class LayoutController extends CController{
	...
	public function actionParty(){
		...
		if(empty($this->serviceInfo['wxAppid'])){
			$url = 'http://www.aodianyun.com/wx/php/token.php?resource_url='.urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
			$signPackage = CModel::curl($url);
		...
		if(empty($signPackage['timestamp'])){
			$url = 'http://www.aodianyun.com/wx/php/token.php?resource_url='.urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
			$signPackage = CModel::curl($url);
			$signPackage = json_decode($signPackage,true);
		}
		...
		$videoUrl = 'http://'.$this->serviceInfo['uid'].'.lssplay.aodianyun.com/'.$partyInfo['lssApp'].'/'.$partyInfo['lssStream'].'.m3u8';
